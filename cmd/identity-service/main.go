// identity-service is the application-plane gRPC binary for the identity
...
// IDENTITY_SERVICE_INSECURE=true.
package main
...
func loadConfig() (config, error) {
	cfg := config{
		Addr:     getenv("IDENTITY_SERVICE_ADDR", ":8085"),
		Insecure: getenvBool("IDENTITY_SERVICE_INSECURE", false),
	}
	...
	if !cfg.Insecure {
		return cfg, errors.New("only IDENTITY_SERVICE_INSECURE=true is supported until SPIRE/SPIFFE wiring lands (ADR-010)")
	}
	return cfg, nil
}
...
func main() {
	...
	log.Printf("identity-service listening on %s (insecure=%v)", cfg.Addr, cfg.Insecure)
