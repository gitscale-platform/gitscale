...
topics:
  - name: gitscale.identity.events
    ...
    acls:
      ...
      # NOTE: Full Terraform ACL wiring is future work. This acls: schema
      # is defined now (ADR-010) so tooling can enforce it once wired.
  ...
  - name: gitscale.billing.events
    partitions: 12
    retention_ms: 604800000          # 7 days — Kafka is operational-only; ClickHouse is the durable audit store (ADR-008).
    key_field: aggregate_id
    rationale: "Usage events trail every metered op. ClickHouse is the durable reconciliation store; Kafka retention is operational-only."
    acls:
      producer_spiffe_id: spiffe://gitscale.dev/ns/data/sa/outbox-consumer
      consumer_spiffe_ids:
...
type EventEnvelope struct {
    ...
    OccurredAt    time.Time       `json:"occurred_at"`     // when the domain event happened (source-of-truth timestamp)
    PublishedAt   time.Time       `json:"published_at"`    // RFC3339, UTC — when outbox consumer published to Kafka
    PrincipalType string          `json:"principal_type"`  // "user" | "agent" | "service" — principal that caused the event
    RateBucket    string          `json:"rate_bucket"`     // billing routing key (e.g. "agent_pro", "human_default")
}
```

**Gap 3 resolution:** `OccurredAt` captures the domain-event timestamp (from the source transaction) distinct from
`PublishedAt` (Kafka publish time). `PrincipalType` enables downstream routing without re-resolving identity.
`RateBucket` is the billing routing key carried in JWT-SVID claims (ADR-010) and forwarded here so the
`BillingAggregator` consumer can route usage records without a secondary lookup.
...
```json
{
  ...
  "type": "object",
  "required": ["event_id", "aggregate_type", "aggregate_id", "event_type", "schema_version", "payload", "occurred_at", "published_at", "principal_type", "rate_bucket"],
  "properties": {
    ...
    "principal_type": { "type": "string", "enum": ["user", "agent", "service"] },
    "rate_bucket": { "type": "string", "minLength": 1 }
  },
