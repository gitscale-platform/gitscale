...
4. **No cross-plane DB calls.** The edge talks to Redis (cache) and the application plane API. It does **not** read PostgreSQL directly.

## Binding ADRs

- **ADR-010** — Service identity via SPIRE/SPIFFE. Per-request JWT-SVID verification. Never trust an upstream-stamped header without re-verifying the SVID at the edge.
- **ADR-009** — Redis for repo-location cache and rate-limit counters, behind the `CacheStore` interface. Cache miss falls through to the application plane API, never directly to PostgreSQL.
...
| Rate limit | Redis `INCR` + `EXPIRE`; fail-closed if Redis unavailable for hard limits, fail-open for soft limits — but log every fail-open as a metric |
| mTLS to upstream | Always present SVID; never `InsecureSkipVerify` |

## Forbidden in this plane

- Direct PostgreSQL connection from a WASM filter
...
- Cross-plane in-process state — every filter must be stateless or backed by Redis
